...
class TestCheckParallel(TestZstash):
    ...
    def helperCheckParallel(
        ...
        cmd = "{}zstash check{} --hpss={} --workers=3".format(
            zstash_path, v_option, self.hpss_path
        )
    ...
    def helperCheckParallelVerboseMismatch(
        ...
        for checksum in output_local.split("\n"):
            checksum = checksum.split("  ")
            if len(checksum) >= 2:
                f_name = checksum[1].split("/")  # remove the backup_dir
    ...
    def testCheckParallelVerboseHPSS(self):
        ...
        self.helperCheckParallel(
            "testCheckParallelVerboseHPSS", HPSS_ARCHIVE, verbose=True
        )
    ...
    def testCheckParallelVerboseMismatch(self):
        self.helperCheckParallelVerboseMismatch(
            "testCheckParallelVerboseMismatch", "none"
        )

    def testCheckParallelVerboseMismatchHPSS(self):
        self.conditional_hpss_skip()
        self.helperCheckParallelVerboseMismatch(
            "testCheckParallelVerboseMismatchHPSS", HPSS_ARCHIVE
        )
