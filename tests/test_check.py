...
class TestCheck(TestZstash):
    ...
    def testCheckKeepTars(self):
        ...
        # Run `zstash create`
        run_cmd('{}zstash create --hpss={} {}'.format(zstash_path, self.hpss_path, self.test_dir))
        ...
        # Run `zstash extract`
        output, err = run_cmd('{}zstash extract --hpss={}'.format(zstash_path, self.hpss_path))
        # Run `zstash check`
        output, err = run_cmd('{}zstash check --hpss={}'.format(zstash_path, self.hpss_path))
        self.assertEqualOrStop(output + err,
