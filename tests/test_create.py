...
class TestCreate(TestZstash):
    ...
    def writeExtraFiles(self):
        ...
        write_file('{}/not_exclude_dir/file_b.txt'.format(self.test_dir), 'file_b_stuff')
        write_file('{}/not_exclude_dir/file_c.txt'.format(self.test_dir), 'file_c stuff')
    ...
    def helperCreateExcludeDir(self, test_name, hpss_path, zstash_path=ZSTASH_PATH):
        ...
        cmd = '{}zstash create --exclude={} --hpss={} {}'.format(zstash_path, excluded_files, self.hpss_path, self.test_dir)
        output, err = run_cmd(cmd)
        expected_present = ['Archiving not_exclude_dir/file_b.txt', 'Archiving not_exclude_dir/file_c.txt']
        if use_hpss:
    ...
    def helperCreateExcludeFile(self, test_name, hpss_path, zstash_path=ZSTASH_PATH):
        ...
        cmd = '{}zstash create --exclude={} --hpss={} {}'.format(zstash_path, excluded_files, self.hpss_path, self.test_dir)
        output, err = run_cmd(cmd)
        expected_present = ['Archiving exclude_dir/file_a.txt', 'Archiving not_exclude_dir/file_c.txt']
        if use_hpss:
            expected_present += ['Transferring file to HPSS']
        else:
            expected_present += ['put: HPSS is unavailable']
